...
export class MainScreen extends GameScreen{
    ...
    prepareRound()
    {
        ...
        let min = this.game.settings?.options[1]?.min ?? 1;
        let max = this.game.settings?.options[1]?.max ?? 10;

        this.count1 = this.game.helper.getRandomInt(min, max);
        this.count2 = this.game.helper.getRandomInt(min, max);

        let operation = this.game.settings?.options[0];

        switch (operation) {
            case 'plus': this.gameResult = this.count1 + this.count2;this.sign = '+';break;
            case 'minus': this.gameResult = this.count1 - this.count2;this.sign = '-';break;
            case 'multiply': this.gameResult = this.count1 * this.count2;this.sign = 'x';break;
        }

        let maxIterations = 0;
        if(operation === 'minus' && this.count2 > this.count1){
            while (this.count2 > this.count1 || maxIterations < 1000){
                this.count1 = this.game.helper.getRandomInt(min, max);
