...
export default class MainScreen extends GameScreen{
    ...
    initScene(){
        ...
        let hero_win = new BaseSprite(this.game.settings['game_path']['img'] + 'win/sova-2.svg','hero-1','win',550,360,240,210,' ');
		let win_text = new BaseSprite(this.game.settings['game_path']['img'] + 'win/molodec.svg','ok-text','win',120,50,588,82,' ');

        let repeat_btn = new BaseSprite(this.game.settings['game_path']['img'] + 'win/repeat-btn.svg','repeat-btn','win',550,190,193,72,' ');
	    ...
	    this.tweens['tweenArrowWin'] = gsap.to(arrow, 1, {_y : "280", repeat: -1, yoyo:true});
        ...
        for(var i=0;i<this.items.length;i++){
            ...
            if(this.items[i].name == 'repeat-btn' && 
                ...
                this.items[i].isShow){
                    ...
                    this.game.reset();
					this.game.screens[0].initScene();
					this.game.screens[0].setTimer();
    ...
    render(){
		...
		this.game.ctx.fillText(this.game.right , 228, 380);
		this.game.ctx.fillText(this.game.wrong , 375, 380);
		this.game.ctx.fillStyle = "#fff";
		this.game.ctx.font = "36pt Arial";
		this.game.ctx.fillText(this.game.points , 274, 290);
